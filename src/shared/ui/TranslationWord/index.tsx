...
export function TranslationWord({ word, remove, index, add }: TranslationWordProps) {
  return (
    <div 
      ...
      onClick={add ? () => add(word) : undefined}
    >
      <div className={`px-3 py-1 rounded bg-popup text-sm border border-zinc-700 hover:bg-zinc-700 transition-colors ${add ? 'opacity-60' : 'opacity-100'}`}>
        {word}
      </div>
